...
void heapRebuild(int rootIndex, Key heap[], int keysCount)
{
	...
	if(!isALeaf(rootIndex, keysCount))
	{
...
istream& operator>> (istream& input, Heap& rightHandSideHeap)
{
	...
	while (index < rightHandSideHeap.heapSize && index < MAXSIZE)
	{
...
void Heap::insert (const Key& newKey) throw (Exception)
{
	...
	while(heapKeys[(currentIndex - 1) / 2] < heapKeys[currentIndex])
	{
		swap(heapKeys[(currentIndex - 1) / 2], heapKeys[currentIndex]);
